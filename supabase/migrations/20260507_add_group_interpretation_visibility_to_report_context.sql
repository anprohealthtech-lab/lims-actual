-- Expose test-group interpretation visibility in report template context.
-- This lets report generation preserve saved HTML while allowing labs to
-- hide it from the final report using test_groups.print_options.showGroupInterpretation.
...
SELECT
    ...
    jsonb_agg(
        DISTINCT jsonb_build_object(
            ...
            'print_options',       tg.print_options,
            'show_group_interpretation_in_report', COALESCE((tg.print_options->>'showGroupInterpretation')::boolean, true),
            'group_interpretation',
              CASE
                WHEN COALESCE((tg.print_options->>'showGroupInterpretation')::boolean, true)
                  THEN tg.group_interpretation
                ELSE NULL
              END
        )
...
COMMENT ON VIEW public.v_report_template_context IS
  'Report template context including test-group interpretation visibility from test_groups.print_options.showGroupInterpretation.';
